...
import filterbtn from './filterIcon.jpg'
...
const [filter,setFilter]=useState({score:'',price:'',sort:'',release_date:'',adult:'',empty:'',name:''})
...
const handleClick= async (e)=>{
 ...
 const response = await axios.get(`http://localhost:8000/api/v1/movies?${filter.adult}&${filter.price}&${filter.name}&${filter.score}&${filter.release_date}&${filter.empty}&${filter.sort}&title=${title.name}`)
...
const secondFetch = async ()=>{
 ...
 try{
 const response = await axios.get(`http://localhost:8000/api/v1/movies?title=${title.name}`)
   ...
   className={styles.search}
     ...
     value={title.name}
      onChange={(e)=>setTitle({name:e.target.value})}
        />
         <img onClick={handleFilter} className={styles.filterIcon} src={filterbtn} width='50px' height='50px' alt='filterIcon'/>
 </div>
  
  {tryBtn&& <Trybtn onSubmited={setFilter} onfilter={filter} FunctionName={handlePage}/>}
 ...
 <div className={styles.movieList}>
                {
                   movies?.map(movie=>(
                        <div className={styles.movieContent} key={movie._id}>
                          ...
                          <img src={IMGPATH+movie.poster_path} width='200px' height='200px' />
                          </Link>
                          <h5>{movie.title}</h5>
